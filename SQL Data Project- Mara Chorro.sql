...
select c.name as Categoria, COUNT(title ) as Recuento 
from film f
...
order by  r.rental_date asc-- 1.99
...
select rating as CATEGORIA,  round (AVG(length ),2) asPROMEDIO_DURACION
from film f 
where length >110
group by rating ;

--21.	¿Cuál es la media de duración del alquiler de las películas?
select AVG(rental_duration )as PROMEDIO_DIAS 
...
select CONCAT (extract (day from R.rental_date), '/',EXTRACT(month from R.RENTAL_DATE),'/', extract(year from R.RENTAL_DATE)) as FECHA_ALQUILER, COUNT(r.INVENTORY_id ) as NUMERO_ALQUILERES 
from rental r 
inner join payment p 
on p.rental_id = r.rental_id
group by  extract (day from R.rental_date),EXTRACT(month from R.RENTAL_DATE), EXTRACT(YEAR from R.RENTAL_DATE)
...
on c.category_id = fc.category_id
group by "name"
having "name" = 'Action';-- 7.143

--51.	Crea una tabla temporal llamada “cliente_rentas_temporalˮ para almacenar el total de alquileres por cliente.
with cliente_rentas_temporal 
as (select concat(c.first_name, ' ', c.last_name) as nombre_cliente , COUNT(rental_id )
from customer c 
inner join rental r 
on c.customer_id =r.customer_id 
...
with peliculas_alquiladas
as (select f.title, COUNT(r.rental_id ) as recuento_peliculas
from rental r
inner join inventory i 
on i.inventory_id = r.inventory_id
inner join film f 
...
on f.film_id = fa.film_id
group by a.actor_id )
select max (recuento ), min (recuento), AVG(recuento )
